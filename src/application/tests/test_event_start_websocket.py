...
@pytest.mark.asyncio
async def test_event_start_receive_broadcast():
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_event_start_broadcast():
    """Test full start event broadcast"""

    organiser = await sync_to_async(CustomUser.objects.first)()
    attendee = await sync_to_async(UserFactory.create)()
    event = await sync_to_async(EventFactory)(organiser=organiser, starting_time=timezone.now(), started=False, participants=(attendee,))
    ...
    await sync_to_async(api_client.post)(f'/api/v1/events/{event.event_id}/start')
