...
@pytest.mark.asyncio
async def test_attendee_leaves_event_group():
    """Test attendee leaving will discard group"""
    ...
    channels = len(channel_layer.groups.keys())
    assert channels > 0

    request = f'{{"request":"leave_event","data":{{"event_id": "{event.event_id}"}},"request_id":2485}}'
    ...
    assert len(channel_layer.groups.keys()) == channels - 1
...
async def test_invalid_event_id_returns_error():
    ...
    assert len(channel_layer.groups.keys()) == channels
